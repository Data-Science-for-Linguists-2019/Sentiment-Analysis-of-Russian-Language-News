# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 14:36:52 2019
...
def preprocess_text(text):
    ...
    russian_stopwords = list(set(stopwords.words("russian") + rs +["который","это", "сказать", "рейтер", "президент", "весь", "год","“","”, - "," “","сообщать",') - ',"”","мочь","также","” ","время",""]))
    mystem = Mystem()
...
folder_path = r'C:\Users\Patrick Stoyer\Dropbox\Documents\Sophomore Year\Semester_2\Ling_1340_Data_Science_for_Linguists\Project\art\reu'
...
lda.save('ldamodel_reuters.gensim')
topics = lda.print_topics(20)
print("Printing topics...")
with open('reuters_lda.txt', 'w', encoding='utf-8') as f:
    for topic in topics:
        print("Topic {}: ".format(topic[0]+1)+"\n\t"+('\n\t'.join([x.strip() for x in topic[1].replace('*',':\t\t').split('+')]))+"\n")
        f.write("Topic {}: ".format(topic[0]+1)+"\n\t"+('\n\t'.join([x.strip() for x in topic[1].replace('*',':\t\t').split('+')]))+"\n")
        
#pickle the preprocessed doc before I forget
files.to_pickle('reuters_lemmatized_series.pkl')
...
lsi.save('lsimodel_reuters.gensim')
topics = lsi.print_topics(20)
print("Printing topics...")
with open('reuters_lsa.txt', 'w', encoding='utf-8') as f:
    for topic in topics:
        print("Topic {}: ".format(topic[0]+1)+"\n\t"+('\n\t'.join([x.strip() for x in topic[1].replace('*',':\t\t').split('+')]))+"\n")
        f.write("Topic {}: ".format(topic[0]+1)+"\n\t"+('\n\t'.join([x.strip() for x in topic[1].replace('*',':\t\t').split('+')]))+"\n")
