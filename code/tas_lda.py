# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 20:44:15 2019
...
folder_path = r'C:\Users\Patrick Stoyer\Dropbox\Documents\Sophomore Year\Semester_2\Ling_1340_Data_Science_for_Linguists\Project\art\tas'
...
lsi.save('lsimodel_tass.gensim')
topics = lsi.print_topics(20)
print("Printing topics...")
with open('tass_lsa.txt', 'w', encoding='utf-8') as f:
    for topic in topics:
        print("Topic {}: ".format(topic[0]+1)+"\n\t"+('\n\t'.join([x.strip() for x in topic[1].replace('*',':\t\t').split('+')]))+"\n")
        f.write("Topic {}: ".format(topic[0]+1)+"\n\t"+('\n\t'.join([x.strip() for x in topic[1].replace('*',':\t\t').split('+')]))+"\n")
